...
async def parse_github_pr_review_comment_trigger(
    payload: dict[str, Any], event_type: str = "pull_request_review_comment"
) -> dict[str, str]:
    """Parse a GitHub inline PR review comment payload for polling triggers."""
    ...
    if not _is_repo_allowed(repo_config):
        logger.warning(
            "Rejecting GitHub PR review comment trigger: repo '%s/%s' not in allowlist",
            repo_config.get("owner"),
            repo_config.get("name"),
        )
        return {"status": "ignored", "reason": "Repository not in allowlist"}
    ...
    if not payload.get("pull_request"):
        return {"status": "ignored", "reason": "No pull request data in review comment"}

    comment = payload.get("comment", {})
    comment_body = comment.get("body") or ""
    if not any(tag in comment_body.lower() for tag in OPEN_SWE_TAGS):
        logger.debug(
            "Ignoring GitHub PR review comment that does not mention @openswe or @open-swe"
        )
        return {"status": "ignored", "reason": "Comment does not mention @openswe or @open-swe"}
