...
from agent.pollers.github import (
    build_issue_comment_payload,
    build_pr_review_comment_payload,
    parse_github_poll_repos,
)
...
async def test_poll_repo_routes_pr_conversation_comments(
    ...
    async def fake_fetch_pr_review_comments_since(*args: object) -> list[dict[str, object]]:
        return []
    ...
    monkeypatch.setattr(
        github_poller,
        "fetch_pr_review_comments_since",
        fake_fetch_pr_review_comments_since,
    )
...
def test_build_pr_review_comment_payload_uses_comment_author_as_sender() -> None:
    payload = build_pr_review_comment_payload(
        "owner",
        "repo",
        {
            "number": 4,
    ...
    assert payload["action"] == "edited"
    assert payload["pull_request"]["number"] == 4
    assert payload["sender"] == {"login": "octocat", "id": 789}


async def test_parse_github_pr_review_comment_trigger_accepts_inline_comment() -> None:
    payload = build_pr_review_comment_payload(
        "owner",
        "repo",
        {
    ...
    assert await webapp.parse_github_pr_review_comment_trigger(payload) == {"status": "accepted"}


async def test_poll_repo_routes_inline_pr_review_comments(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    called: dict[str, object] = {}
    ...
    async def fake_set_cursor(namespace: tuple[str, ...], key: str, value: str) -> None:
        called.setdefault("cursors", []).append((key, value))
    ...
    async def fake_fetch_pull_request(*args: object) -> dict[str, object]:
        return {
            "number": 4,
            "html_url": "https://github.com/owner/repo/pull/4",
            "head": {"ref": "feature-branch"},
    ...
    async def fake_process_github_issue(payload: dict, event_type: str) -> None:
        raise AssertionError("issue processor should not handle inline PR review comments")
    ...
    monkeypatch.setattr(github_poller, "fetch_pull_request", fake_fetch_pull_request)
    monkeypatch.setattr(github_poller, "process_github_pr_comment", fake_process_github_pr_comment)
    monkeypatch.setattr(github_poller, "process_github_issue", fake_process_github_issue)

    await github_poller.poll_repo("owner", "repo", "token")

    assert called["event_type"] == "pull_request_review_comment"
    assert called["payload"]["pull_request"]["head"]["ref"] == "feature-branch"
